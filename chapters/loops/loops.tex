...
Imagine que você precisa imprimir os números de 1 a 1000. Seria tedioso escrever 1000 \textit{printf} statements, não é? Para situações assim existem as estruturas de repetição, também conhecidas como loops.

Um loop permite executar um bloco de código múltiplas vezes. Existem basicamente três tipos em C.
...
\begin{quote}
\textit{Cuidado com loops infinitos!}
\end{quote}

Se você esquecer de atualizar a variável da condição, o loop nunca terminará:
...
O \textit{do-while} é similar ao \textit{while}, mas garante que o bloco execute AT LEAST uma vez, mesmo que a condição seja falsa inicialmente.
...
Este código imprime \say{Número: 10} uma vez, mesmo que \textit{numero} (10) não seja menor que 5.

Com \textit{while} normal, nada seria impresso.
