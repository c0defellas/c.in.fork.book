...
Um erro comum é usar uma única igualdade (\textit{=}) em vez de duas (\textit{==}). A primeira é atribuição, a segunda é comparação.

\begin{ccode}
  if (x = 5) { }    // ERRADO: atribui 5 a x
  if (x == 5) { }   // CORRETO: compara x com 5
\end{ccode}
...
\begin{quote}
\textit{Nunca esqueça do break!}
\end{quote}

O \textit{break} interrompe a execução do \textit{switch}. Sem ele, a execução continua para o próximo \textit{case}, mesmo que a condição não seja atendida:
